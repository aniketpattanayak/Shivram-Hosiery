...
const CreateSampleModal = ({ onClose, onSuccess, products, materials }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    originalProductId: "",
    // NEW FIELDS
    category: "", // Changed default to empty for dynamic input
    subCategory: "",
    fabricType: "",
    color: "",
  });
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg border border-slate-100 shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
          ...
          <button onClick={onClose}>
            <FiPlus className="rotate-45 text-slate-400 hover:text-red-500 transition-colors" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          {type === "Existing Product" && (
            <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 space-y-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      originalProductId: e.target.value,
                    })
                  }
                >
              ...
              <div>
                ...
                <input
                  className="w-full border border-purple-200 p-2.5 rounded-lg mt-1 text-sm"
                  placeholder="e.g. Red (Original was Blue)"
              ...
              <div className="relative">
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    className="w-full border p-2 rounded-lg text-sm pl-8 focus:ring-2 focus:ring-blue-500/20 outline-none"
              ...
              <div className="max-h-32 overflow-y-auto pr-1 custom-scrollbar">
                {manualBom.map((row, idx) => (
                  <div key={idx} className="flex gap-2 mb-2">
                    <select
                      className="flex-1 border p-1.5 rounded-lg text-xs bg-slate-50"
                      value={row.material}
                      onChange={(e) =>
                    ...
                    <input
                      type="number"
                      className="w-20 border p-1.5 rounded-lg text-xs bg-slate-50"
                      placeholder="Qty"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                className="w-full border border-slate-200 p-2.5 rounded-lg mt-1 text-sm"
          ...
          <button
            ...
            className="w-full bg-slate-900 text-white font-bold py-3.5 rounded-xl hover:bg-slate-800 shadow-lg shadow-slate-200 transition-transform active:scale-[0.98]"
          >
            Create Sample
          </button>
        </div>
...
export default function SamplingPage() {
  ...
  const handleIssueMaterial = async (sampleId) => {
    ...
    try {
      ...
      alert("Stock Error: " + e.response?.data?.msg);
    }
  };
      ...
      <div className="flex justify-between items-end pb-4 border-b border-slate-200">
        <div>
          ...
          <p className="text-slate-500 mt-2">
            Design, Prototype, and Approve new products.
          </p>
        </div>
        <button
          onClick={() => setShowModal(true)}
          className="bg-slate-900 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-blue-600 transition-colors flex items-center gap-2 shadow-lg shadow-slate-200"
        >
      ...
      <div className="flex-1 overflow-x-auto pb-4">
        <div className="flex gap-4 min-w-max h-full">
          {stages.map((stage) => (
            <div
              key={stage}
              className="w-72 flex flex-col bg-slate-100/80 rounded-2xl p-3 border border-slate-200/60 backdrop-blur-sm"
            >
              <div className="flex items-center gap-2 mb-3 px-2">
                <span
                  className={`w-2 h-2 rounded-full ${
                    stage === "Approved" ? "bg-green-500" : "bg-slate-400"
                  }`}
                ></span>
                      ...
                      <div className="flex gap-2 mt-2 flex-wrap">
                        {sample.color && (
                          <span className="text-[10px] bg-slate-50 text-slate-600 px-1.5 py-0.5 rounded border border-slate-100 flex items-center gap-1">
                            <FiLayers size={8} /> {sample.color}
                          </span>
                        )}
                        {sample.fabricType && (
                          <span className="text-[10px] bg-slate-50 text-slate-600 px-1.5 py-0.5 rounded border border-slate-100 flex items-center gap-1">
                            <FiTag size={8} /> {sample.fabricType}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="pt-3 mt-3 border-t border-slate-100 flex gap-2">
                        {stage === "Pattern" && !sample.materialsIssued && (
                          <button
                            ...
                            className="flex-1 bg-amber-50 text-amber-700 text-xs font-bold py-2 rounded-lg hover:bg-amber-100 flex justify-center gap-1 items-center"
                          >
                        ...
                        {stage === "Approved" && !sample.convertedProductId ? (
                          <button
                            ...
                            className="flex-1 bg-emerald-600 text-white text-xs font-bold py-2 rounded-lg hover:bg-emerald-700 flex justify-center gap-1 items-center shadow-emerald-200 shadow-sm"
                          >
                          ...
                          <button
                            onClick={() => handleNextStage(sample)}
                            className="flex-1 bg-slate-50 text-slate-600 text-xs font-bold py-2 rounded-lg hover:bg-slate-900 hover:text-white transition-colors flex justify-center gap-1 items-center"
                          >
