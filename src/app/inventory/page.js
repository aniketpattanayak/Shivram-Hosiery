...
export default function InventoryPage() {
  ...
  const handleRecalculate = async () => {
      if(!confirm("This will update Health Status for all items based on the new formula. Proceed?")) return;
  ...
  const getStatusBadge = (status) => { 
    ...
    if (s === "CRITICAL") return <span className="px-3 py-1 rounded-full text-[10px] font-black bg-red-50 text-red-600 border border-red-100">CRITICAL</span>;
    if (s === "MEDIUM") return <span className="px-3 py-1 rounded-full text-[10px] font-black bg-orange-50 text-orange-600 border border-orange-100">MEDIUM</span>;
    if (s === "OPTIMAL") return <span className="px-3 py-1 rounded-full text-[10px] font-black bg-emerald-50 text-emerald-600 border border-emerald-100">OPTIMAL</span>;
    if (s === "EXCESS") return <span className="px-3 py-1 rounded-full text-[10px] font-black bg-purple-50 text-purple-600 border border-purple-100">EXCESS</span>;
    return <span className="px-3 py-1 rounded-full text-[10px] font-black bg-slate-100 text-slate-500">{s}</span>;
  };
                ...
                <tr key={item._id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-mono font-bold text-slate-700 text-lg">
                        {Number(item.current).toFixed(2)} <span className="text-[10px] text-slate-400 font-bold">{item.unit}</span>
                    </div>
                    {item.batches && item.batches.length > 0 && (
                        <button onClick={() => setViewStockItem(item)} className="text-[10px] text-blue-600 font-bold hover:underline flex items-center gap-1 mt-1 bg-blue-50 px-1.5 py-0.5 rounded w-fit">
                     ...
                     <div className="w-full">
                        <div className="flex justify-between items-end mb-1">
                            <span className={`text-xs font-black ${
                                item.health <= 33 ? 'text-red-600' : 
                                item.health <= 66 ? 'text-amber-500' : 
                                item.health <= 100 ? 'text-emerald-600' : 
                                'text-purple-600'
                            }`}>
                                {item.health.toFixed(2)}%
                            </span>
                            {item.health <= 33 && <FiActivity className="text-red-500 animate-pulse" size={12}/>}
                        </div>
                        <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                            <div 
                                className={`h-full rounded-full transition-all duration-500 ${
                                    item.health <= 33 ? 'bg-red-500' : 
                                    item.health <= 66 ? 'bg-amber-400' : 
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-200 flex flex-col max-h-[80vh]">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                    <div>
                        <h3 className="font-bold text-lg text-slate-900 flex items-center gap-2"><FiPackage className="text-blue-600"/> Batch / Lot Details</h3>
                            ...
                            {viewStockItem.batches && viewStockItem.batches.length > 0 ? (
                                viewStockItem.batches.map((batch, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50">
                                        <td className="px-6 py-3 font-mono text-xs font-bold text-blue-600 bg-blue-50/30">{batch.lotNumber}</td>
                                        <td className="px-6 py-3 text-xs text-slate-500 flex items-center gap-1"><FiCalendar size={10} /> {new Date(batch.addedAt || Date.now()).toLocaleDateString()}</td>
                                        <td className="px-6 py-3 text-right font-black text-slate-800">{batch.qty}</td>
                                    </tr>
                                ))
                            ) : (
                                <tr><td colSpan="3" className="px-6 py-8 text-center text-slate-400 italic">No active batches found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
