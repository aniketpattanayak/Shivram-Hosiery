    ...
    <div className="min-h-screen bg-slate-100 p-8 print:p-0 print:bg-white">
      ...
      <div className="max-w-4xl mx-auto bg-white p-12 shadow-2xl rounded-xl print:shadow-none print:rounded-none print:w-full print:max-w-none">
        ...
        <div className="grid grid-cols-2 gap-12 mb-10">
          <div>
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Quotation For</h3>
            <p className="text-lg font-bold text-slate-900">{quote.clientName}</p>
            <p className="text-sm text-slate-600 whitespace-pre-line mt-1">{quote.clientAddress}</p>
            {quote.clientGst && <p className="text-sm font-bold text-slate-800 mt-2">GSTIN: {quote.clientGst}</p>}
          </div>
          <div className="text-right">
            <div className="mb-4">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Date</h3>
              <p className="font-bold text-slate-900">{new Date(quote.createdAt).toLocaleDateString()}</p>
            </div>
            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Valid Until</h3>
              <p className="font-bold text-red-600">{new Date(quote.validUntil).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
        ...
        {quote.subject && (
          <div className="mb-8 p-4 bg-slate-50 rounded-lg border border-slate-100">
            <span className="font-bold text-slate-700">Subject:</span> <span className="text-slate-600">{quote.subject}</span>
          </div>
        )}
        ...
        <table className="w-full text-left mb-8">
          <thead>
            <tr className="border-b-2 border-slate-100">
              <th className="py-3 text-xs font-bold text-slate-500 uppercase">Item & Description</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {quote.items.map((item, index) => (
              <tr key={index}>
                ...
                <td className="py-4 text-right font-medium text-slate-700">{item.qty}</td>
                <td className="py-4 text-right font-medium text-slate-700">₹{item.rate.toLocaleString()}</td>
        ...
        <div className="border-t border-slate-200 pt-8 grid grid-cols-2 gap-8">
          <div>
            <h3 className="font-bold text-slate-900 mb-3 text-sm">Terms & Conditions</h3>
            <ul className="text-xs text-slate-500 space-y-1 list-disc pl-4">
              <li><strong>Payment:</strong> {quote.terms?.payment || 'N/A'}</li>
              <li><strong>Delivery:</strong> {quote.terms?.delivery || 'N/A'}</li>
              <li><strong>Validity:</strong> {quote.terms?.validity || '30 Days'}</li>
            </ul>
          </div>
