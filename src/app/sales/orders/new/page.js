...
import {
  FiPlus, FiTrash2, FiSave, FiUser, FiCalendar, FiBox, 
  FiFileText, FiArrowLeft, FiCheckCircle, FiAlertCircle, FiArrowDownCircle 
} from "react-icons/fi";
...
const OrderItemCard = ({ index, item, onChange, onRemove, productList }) => {
  return (
    <div className="bg-slate-50 rounded-xl border border-slate-200 p-5 relative group transition-all hover:shadow-md hover:border-blue-300">
      ...
      {/* Row 1: Product Search (Big Input) */}
      ...
      {/* Row 2: Specs (Auto-Filled Gray Boxes) */}
...
export default function NewOrderPage() {
  ...
  const financials = useMemo(() => {
    ...
    const taxAmount = subTotal * 0.18; // Assuming 18% GST standard
  ...
  const handleApplyQuote = (quoteId) => {
    ...
    const mappedItems = selectedQuote.items.map(qItem => {
        // Try to find specs from Product Master to fill the gray boxes
  ...
  const handleItemChange = (index, field, value) => {
    ...
    // 🟢 AUTO-FILL SPECS & PRICE on Product Select
    if (field === 'productName') {
        ...
        if (product) {
            ...
            newItems[index].unitPrice = product.sellingPrice || 0; // Auto-Set Price
        }
    }
  ...
  const addItem = () => setFormData(prev => ({ 
      ...prev, 
      items: [...prev.items, { productName: "", qtyOrdered: 1, unitPrice: 0 }] 
  }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        router.push('/sales/orders'); // Redirect to List
      }
    } catch (error) {
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500 pb-20 p-6">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900">New Sales Order</h1>
          <p className="text-slate-500 text-sm">Generate a formal order contract.</p>
        </div>
      </div>

      {/* Won Lead Notification */}
      {wonLeads.length > 0 && (
        <div className="mb-8 bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex flex-wrap gap-4 items-center">
            ...
            {wonLeads.map(lead => (
                <button 
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 bg-white border border-indigo-200 text-indigo-700 text-xs font-bold rounded-lg hover:shadow-md transition-all"
                >
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {clientQuotes.length > 0 ? (
                    <div>
                        ...
                        <select 
                            ...
                            className="w-full p-3 bg-purple-50 border border-purple-200 rounded-xl font-bold text-purple-800 focus:ring-2 focus:ring-purple-200 outline-none"
                        >
                    ...
                    <div className="flex bg-slate-100 rounded-xl p-1">
                        {['Low', 'Medium', 'High'].map(p => (
                            <button 
                                ...
                                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${formData.priority === p ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                {p}
                            </button>
                        ))}
        ...
        {/* SECTION 2: LINE ITEMS (STACKED) */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-6">
                <h3 className="font-bold text-slate-800 flex items-center gap-2">
                    <div className="p-2 bg-emerald-50 text-emerald-600 rounded-lg"><FiBox /></div> Order Items
                </h3>
                <button type="button" onClick={addItem} className="text-xs font-bold bg-slate-900 text-white px-4 py-2 rounded-lg hover:bg-black transition-colors flex items-center gap-2">
                    <FiPlus /> Add Item
                </button>
            </div>
        ...
        <div className="flex justify-end">
            <div className="w-full md:w-80 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                ...
                <div className="border-t border-slate-200 pt-3 flex justify-between items-center">
                    ...
                    <span className="text-2xl font-black text-slate-900">₹{financials.grandTotal.toLocaleString()}</span>
                </div>
            </div>
        </div>
        ...
        <div className="pt-4">
            <button 
                type="submit" 
                disabled={loading}
                className="w-full py-4 bg-slate-900 text-white font-bold text-lg rounded-2xl shadow-xl hover:bg-black transition-transform active:scale-[0.99] flex justify-center items-center gap-3"
            >
