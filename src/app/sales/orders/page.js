...
import {
  ...
  FiArrowDownCircle, FiClock, FiTruck, FiShoppingCart, FiCalendar
} from "react-icons/fi";
    ...
    <div className="bg-slate-50 rounded-xl border border-slate-200 p-5 relative group transition-all hover:shadow-md hover:border-blue-300">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
        <div>
            ...
            <div className="bg-slate-200/50 text-slate-600 font-medium text-xs px-3 py-2 rounded-lg border border-slate-200 h-9 flex items-center">{item.category || '-'}</div>
        </div>
        <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase">Sub-Cat</label>
            <div className="bg-slate-200/50 text-slate-600 font-medium text-xs px-3 py-2 rounded-lg border border-slate-200 h-9 flex items-center">{item.subCategory || '-'}</div>
        </div>
        <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase">Fabric</label>
            <div className="bg-slate-200/50 text-slate-600 font-medium text-xs px-3 py-2 rounded-lg border border-slate-200 h-9 flex items-center">{item.fabricType || '-'}</div>
        </div>
        <div>
        ...
        <div>
            ...
            <input 
                ...
                onChange={(e) => onChange(index, "promiseDate", e.target.value)}
                className="w-full bg-white border border-blue-200 text-slate-800 font-bold text-xs rounded-lg px-2 py-2 h-9 focus:ring-2 focus:ring-blue-500 outline-none"
            />
        </div>
      </div>
...
export default function NewOrderPage() {
  ...
  const fetchQuotesForClient = async (name) => {
    try {
        const res = await api.get('/sales/quotes'); 
        const matchedQuotes = res.data.filter(q => q.clientName === name);
    ...
    try {
      ...
      if (res.data.success) {
        if (payload.customerId) {
            try { await api.put(`/sales/clients/${payload.customerId}`, { status: 'Customer' }); } catch (e) {}
        }
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500 pb-20 p-6">
      ...
      <div className="flex items-center gap-4 mb-8">
        <div><h1 className="text-3xl font-extrabold text-slate-900">Sales Order Entry</h1><p className="text-slate-500 text-sm">Generate new demand and view history.</p></div>
      </div>

      {wonLeads.length > 0 && (
        <div className="mb-8 bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex flex-wrap gap-4 items-center">
            <span className="text-xs font-bold text-indigo-700 uppercase tracking-wide flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span> Won Leads:</span>
            {wonLeads.map(lead => (<button key={lead._id} onClick={() => handleConvertLead(lead)} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-indigo-200 text-indigo-700 text-xs font-bold rounded-lg hover:shadow-md transition-all">{lead.name} <FiArrowDownCircle /></button>))}
        </div>
      )}

      {status.msg && (<div className={`mb-6 p-4 rounded-xl text-sm font-bold flex items-center gap-2 ${status.type === 'error' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>{status.type === 'error' ? <FiAlertCircle size={18}/> : <FiCheckCircle size={18}/>} {status.msg}</div>)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        required placeholder="Search existing or type new..."
                    />
                    <datalist id="customer-list">
                        {allClients.map(c => <option key={c._id} value={c.name}>{c.gstNumber ? `GST: ${c.gstNumber}` : 'Lead'}</option>)}
                    </datalist>
                </div>
                ...
                <div>
                    ...
                    <div className="flex bg-slate-100 rounded-xl p-1">
                        {['Low', 'Medium', 'High'].map(p => (<button key={p} type="button" onClick={() => setFormData({...formData, priority: p})} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${formData.priority === p ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}>{p}</button>))}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-xs border-b border-slate-200">
                <tr><th className="p-4">Order ID</th><th className="p-4">Customer</th><th className="p-4">Date</th><th className="p-4">Status</th><th className="p-4 text-center">Items</th><th className="p-4">Priority</th><th className="p-4 text-right">Total</th></tr>
              </thead>
                ...
                {orders.length > 0 ? (
                    orders.map((order) => (
                      <tr key={order._id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-4"><span className={`px-2 py-1 rounded text-xs font-bold border ${order.status === 'Production_Queued' ? 'bg-purple-50 text-purple-700 border-purple-200' : order.status === 'Ready_Dispatch' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :'bg-slate-100 text-slate-600 border-slate-200'}`}>{order.status}</span></td>
                        <td className="p-4 text-center font-mono font-bold text-slate-600">{order.items?.length || 0}</td>
                        <td className="p-4"><span className={`px-2 py-1 rounded-full text-[10px] uppercase font-bold ${order.priority === 'High' ? 'bg-red-100 text-red-700' : order.priority === 'Low' ? 'bg-slate-100 text-slate-600' : 'bg-blue-50 text-blue-700'}`}>{order.priority}</span></td>
